...
function kMeansClustering($data, $k = 3, $fields = [], $maxIter = 100) {
    ...
    foreach ($assignments as $i => $c) {
        $sum = 0;
        foreach ($fields as $f) { $sum += $data[$i][$f.'_norm']; }
    ...
    $colorList = ['#059669', '#d97706', '#6366f1'];
    $iconList = ['high', 'medium', 'low'];
    foreach ($rank as $ri => $ci) {
        $labels[$ci] = $labelNames[$ri];
        $colors[$ci] = $colorList[$ri];
        $icons[$ci] = $iconList[$ri];
    }
